  ...
  (ANY "/wave"
    (log (let [events (log (params :events))]
	   (if (or (.contains events "\"name\":\"we/eval\"")
	     ...
	     (json-str (log-info "Bounced" (wrap-json-operations-with-bundle (workaround-bug [])))))))))
...
(defn-log add-to-class [partition class el] 
  (conj (disj1 partition class) (conj class el)))
...
(defn-log replicate-replocs! [r1 r2]
  (let [rc1 (containing-rep-class r1) rc2 (containing-rep-class r2)]
    (cond
      ...
      (swap! rep-rules #(conj (disj1 % rc1 rc2) (concat rc1 rc2))))))
...
(def workaround-bug1-store (atom '()))

(def *bounce* true)

;   "There is a bug right now (or so we think) that you can't send more than one DOCUMENT_ELEMENT_MODIFY_ATTRS
;operation in one response. In order to make things work, we will store any additional such operations in an atom
;which will be read on a subsequent call"
(defn-log workaround-bug1
  [ops]
  (let [element-modify-ops 
	(for [op ops :when (= (op "type") "DOCUMENT_ELEMENT_MODIFY_ATTRS")] 
	  op)]
    (cond
      (and (= (count element-modify-ops) 0) *bounce*)
      (let [new-op (first @workaround-bug1-store)]
	(if new-op
	  (do
	    (swap! workaround-bug1-store rest)
	    (conj ops new-op))
	  ops))
     
      (> (count element-modify-ops) 1)
      (let [remove-ops (rest element-modify-ops)]
	(swap! workaround-bug1-store concat remove-ops)
	(filter (complement (set remove-ops)) ops))
      
      :else
      ops)))

(defn-log clear-nil-values [m]
  (into {} (for [[k v] m :when v] [k v])))

; Removes any map keys containing nil values from DOCUMENT_ELEMENT_MODIFY_ATTRS
; operations
(defn-log workaround-bug2 [ops]
  (for [op ops] (update-in op op-map-path clear-nil-values)))

; Never send a DOCUMENT_ELEMENT_MODIFT_ATTRS with no delta
(defn-log workaround-bug3 [ops]
  (for [op ops :when (> (count (op op-map-path) 1))] op))
...
(defn workaround-bug [ops]
  (workaround-bug3 (workaround-bug2 (workaround-bug1 ops))))

(defn-log do-replication-by-json [events-map]
  (workaround-bug (unite-gadget-modifications (mapcat rep-op-to-operations (do-replication @rep-rules (incoming-map-to-rep-ops events-map))))))
...
(defn a [] (remember-gadget-key! "_view.html"))
(defn b [] (replicate-gadget-key! "f1._view.html"))
